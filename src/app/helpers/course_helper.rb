module CourseHelper
  def requiresCourse(course_id)
    unless (Course.find_all_by_user_id(current_user.id).include?(Course.find_by_id(course_id))||Course.find_by_id(course_id).get_students.include?(current_user.id))
